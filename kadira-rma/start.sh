...
dumpEnvVarsTo() {
  ...
  MONGO_APP_CONN=$(pick-mongo-primary-one $MONGO_URL)
...
while [[ true ]]; do
  ...
  if [ -z ${START_TIME+x} ] || [ -z ${END_TIME+x} ]; then
      mongo $(pick-mongo-primary-one $MONGO_METRICS_URL) profiles/$PROFILE.js providers/$PROVIDER.js $ENV_FILE_NAME lib.js mapreduce.js incremental-aggregation.js;
  else
      mongo $(pick-mongo-primary-one $MONGO_METRICS_URL) profiles/$PROFILE.js providers/$PROVIDER.js $ENV_FILE_NAME lib.js mapreduce.js batch-aggregation.js;
  fi
  set +x;
  completedAt=$(date +%s)
